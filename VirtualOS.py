...
from virtualkernel import VirtualKernel
                                                                                   class VirtualOS:                                                                       def __init__(self):                                                                    self.kernel = VirtualKernel()
        ...
        if "home" in self.fs.root.subdirectories:                                              # Check if 'user' directory exists
            ...
            self.kernel.log_command("Home directory not found. Creating...")                   self.fs.create_directory("/home")
            self.fs.create_directory("/home/user")
            self.current_directory = self.fs.root.subdirectories["home"].subdirectories["user"]
        ...
        except Exception as e:                                                                 self.kernel.log_command(f"Error during kernel boot: {str(e)}")

    def load_with_loading_circle(self):
        self.kernel.log_command("Loading with loading circle...")
        loading_animation = ['|', '/', '-', '\\']  # ASCII characters for the loading animation                                                                               for _ in range(10):  # Repeat the animation 10 times
    ...
    def run_shell(self):
        while True:
            try:
                ...
                if command == "exit":                                                                  print("Exiting VirtualOS...")                                                      self.fs.save_file_system("file_system.json")  # Save filesystem
                    ...
                    print("Command not found. Available commands: mkdir, ls, cd, cat, rmdir, nano, version, clear, run_vm, dmesg, toggle_fs_monitoring, monitor_fs, exit")                                                                                                   self.kernel.log_command(f"[!] Command '{command}' not found.")
